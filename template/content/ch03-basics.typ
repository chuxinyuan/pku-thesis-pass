...
#strong[注意]：本模板默认允许表格跨页显示（`show figure: set block(breakable: true)`）。长表跨页时会自动重复表头，并在续表页居中显示完整表题（在表号前加"续"，如"续表 1.1 三线表示例"），无需手动控制。如果不希望某个表格被分割，可以在表格前手动插入 `#pagebreak()` 进行调整。详细 API 见 @advanced 的「组件与辅助函数参考」。
