...
书脊页用于打印装订时在书脊上显示论文标题与作者，北大规范未强制要求，但博士论文装订常见。如打印论文时需启用，在 `thesis.typ` 中取消注释 `#spine()` 即可#footnote("建议打印时同时设置章节总是从奇数页开始，方法是设置 always-start-odd: true。")。
...
如果论文中需要“主要符号对照表”，可以在 `thesis.typ` 中取消注释 `#notation`所在行即可。例如：
...
盲审模式下（`blind: true`），论文封面会自动切换为盲审版，隐藏作者、导师、学号、"攻读学位期间发表的论文"、致谢、及原创性声明，PDF 元数据隐藏作者。盲审模式下封面显示 `blind-id`，而非 `author-zh`。

因此#strong[你不需要手动删除或者注释以上要素的代码]，只需要在 `config()` 中设置 `blind: true` 即可。
